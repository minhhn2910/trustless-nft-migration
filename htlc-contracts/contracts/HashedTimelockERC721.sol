...
contract HashedTimelockERC721 {
    ...
    modifier withdrawable(bytes32 _contractId) {
        // No need for this check, the end point of receiving token must always be c.receiver regardless of who call the function
    ...
    * @return bool true on success
        ...
        returns (bool)
    {
        ...
        c.withdrawn = true;
        c.blocknumber = block.number; //for faster verifying the burn tx
        ...
        )
    {
        if (haveContract(_contractId) == false)
            return (address(0), address(0), address(0), 0, 0, 0, false, false, 0, 0, 0);
        LockContract storage c = contracts[_contractId];
        return (
            c.sender,
            c.receiver,
